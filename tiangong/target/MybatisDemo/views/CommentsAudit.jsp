      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu" style="display: block;">
            ...
            <li class="active"><a href="${APP_PATH }/views/CommentsAudit.jsp">评论审核</a></li>
...
<script src="${APP_PATH }/bower_components/jquery/dist/jquery.min.js"></script>
  <script type="text/javascript" >
	...
	function to_page(pn){
		//console.log("调用。。。。")
		$.ajax({
			url:"${APP_PATH}/check/commentsList",
			data:"pn="+pn,
			type:"GET",
	...
	function build_emps_table(result){
		...
		$.each(admins,function(index,item){
			...
			var adminNameTd = $("<td class='text-center'></td>").append(item.content);
			...
			.append(passBtn).append(" ").append(notPassBtn);
			//var delBtn = 
	...
	$(document).on("click",".pass_btn",function(){
		...
		if(confirm("确定这么做吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/check/commentsAudit?result=false&id="+Id,
				type:"PUT",
				success:function(result){
					swal({
		                title: '操作成功！',
	...
	$(document).on("click",".nopass_btn",function(){
		...
		if(confirm("确定这么做吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/check/commentsAudit?result=true&id="+Id,
				type:"PUT",
				success:function(result){
					swal({
		                title: '操作成功！',
