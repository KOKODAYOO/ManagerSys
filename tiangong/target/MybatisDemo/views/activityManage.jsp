...
<div class="wrapper">
  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="active"><a href="${APP_PATH }/views/activityManage.jsp"><i class="fa fa-link"></i> <span>线下活动管理</span></a></li>
      </ul>
      <!-- /.sidebar-menu -->
    </section>
    <!-- /.sidebar -->
 ...
 <div class="modal fade" id="memberModal" role="dialog" aria-labelledby="videoPlayModal" aria-hidden="true" >
    <div class="modal-dialog" style="width:960px;">
    <div class="modal-content" style="border: 0px;border-radius: 0px;">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      	<h4 class="modal-title" id="myModalLabel">活动成员</h4>
                ...
                <table id="content2"  class="table table-striped m-b-0">
                  <thead>
                  <tr>
...
<!-- Content Header (Page header) -->
  ...
  <script type="text/javascript" >
	...
	function to_page(pn){
		//console.log("调用。。。。")
		$.ajax({
			url:"${APP_PATH}/check/activityList",
			data:"pn="+pn,
			type:"GET",
	...
	function build_emps_table(result){
		...
		$.each(admins,function(index,item){
			...
			var finishTd = $("<td class='text-center'></td>").append(item.is_finish==true?"结束":"进行");
			...
			var viewBtn =  $("<button data-toggle='control-sidebar'></button>").addClass("btn btn-info btn-sm view_btn")
							.append($("<span></span>").addClass("glyphicon glyphicon-eye-open")).append("查看详情");
			
			var viewmBtn =  $("<button data-toggle='control-sidebar'></button>").addClass("btn btn-info btn-sm viewm_btn")
			.append($("<span></span>").addClass("glyphicon glyphicon-user")).append("查看成员");
			viewmBtn.attr("id",item.id);
			...
			.append(viewmBtn).append(" ").append(viewBtn).append(" ").append(notPassBtn);
			//var delBtn = 
	...
	$(document).on("click",".nopass_btn",function(){
		...
		if(confirm("确定这么做吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/check/deleteAct?id="+Id,
				type:"DELETE",
				success:function(result){
					swal({
		                title: '操作成功！',
	...
	$(document).on("click",".view_btn",function(){
		...
		$.ajax({
			url:"${APP_PATH}/communicate/getActivityById?id="+id,
			type:"GET",
			success:function(result){
					var content = result.data.activity.content;
					$("#desc").val(content);
					var pictures = result.data.activity.pictures;
					//console.log(admins);
					$.each(pictures,function(index,item){
						//console.log(item);
						var picture = $("<img src='"+item.url+"' class='img-thumbnail'>");
						//var delBtn = 
						//append方法执行完成以后还是返回原来的元素
						$("<div><div>")
							.append(picture)
	...
	$(document).on("click",".viewm_btn",function(){
		
		var id = $(this).attr("id");
		//document.getElementById('container').innerHTML = content;
		$("#content2 tbody").empty();
		
		$.ajax({
			url:"${APP_PATH}/communicate/getMembers?id="+id,
			type:"GET",
			success:function(result){
					...
					$.each(members,function(index,item){
						...
						//var picture = $("<td class='text-center'></td>").append("<div class='pull-left image'></div>").appand("<img src='"+item.avatarUrl+"' class='img-circle' alt='User Image'>");
						//var name =  $("<td class='text-center'></td>").append("<div class='pull-left info'></div>").append(item.name);
						//var delBtn = 
						//append方法执行完成以后还是返回原来的元素
						$("<tr></tr>")
						.append(adminIdTd)
						.append(adminNameTd)
						.appendTo("#content2 tbody");
