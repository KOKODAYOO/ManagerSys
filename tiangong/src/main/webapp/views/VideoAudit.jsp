      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu " style="display: block;">
            ...
            <li class="active"><a href="${APP_PATH }/views/VideoAudit.jsp">视频审核</a></li>
  ...
  <script type="text/javascript" >
	...
	function build_emps_table(result){
		...
		$.each(admins,function(index,item){
			...
			viewBtn.attr("content",item.video_url);
	...
	$(document).on("click",".pass_btn",function(){
		...
		if(confirm("确定这么做吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/check/videoAudit?result=true&id="+Id,
				type:"PUT",
				success:function(result){
					swal({
		                title: '操作成功！',
	...
	$(document).on("click",".nopass_btn",function(){
		...
		if(confirm("确定这么做吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/check/videoAudit?result=false&id="+Id,
				type:"PUT",
				success:function(result){
					swal({
		                title: '操作成功！',
